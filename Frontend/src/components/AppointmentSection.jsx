...
export default function AppointmentSection() {
  ...
  const fetchSlots = async () => {
    try {
      const ids = formData.serviceIds.join(",");
      const res = await fetch(
        `${API_URL}/appointments/available-slots/${formData.date}?serviceIds=${ids}`
      );
      const data = await res.json();
  ...
  const handleSubmit = async (e) => {
    ...
    const { name, email, phone, serviceIds, date, time } = formData;
    if (!name || !email || !phone || serviceIds.length === 0 || !date || !time) {
      setError("Please fill all fields and select at least one service");
      return;
    }
        ...
        >
          ...
          <div className="relative w-full max-w-2xl">
            ...
            >
              ...
              ========================== */}
              <div className="md:col-span-2 bg-white text-black px-4 py-3 rounded">
                  ...
                  {services.map((s) => (
                    ...
                    >
                      <input
                        ...
                        className="w-4 h-4 accent-[#BB8C4B]"
                      />
              ...
              <select
                ...
                value={formData.time}
                onChange={handleChange}
                disabled={formData.serviceIds.length === 0 || availableSlots.length === 0}
                className="h-[56px] bg-white text-black px-6"
              >
                <option value="">
